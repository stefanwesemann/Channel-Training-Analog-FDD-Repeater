# This script is used to generate the right side of Figure 5, in the article:
#
...
if (plotFileFlag){
    scaleFactor <- 0.9
    pdf("beta_var.pdf", width=scaleFactor*8, height=scaleFactor*6)
}
...
p <- p + xlab(expression(paste(beta," (dB)")))+ 
    ylab(expression(paste(beta^{-2},E(hat(beta)-E(hat(beta)))^2," (dB)")))
